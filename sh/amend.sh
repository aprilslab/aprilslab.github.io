...
if [ -n $N ] && [ $N -eq $N ] 2>/dev/null; then
  ...
  # echo $@
	git add .
	GIT_AUTHOR_DATE=$N_DAYS_BEFORE GIT_COMMITTER_DATE=$N_DAYS_BEFORE git commit --amend -m "$M"
else
  TODAY=$(TZ=KST date -d +"9 hour" +"%Y-%m-%dT%H:%M:%S")
  echo $TODAY # For test
  echo $@
  git add .
  GIT_AUTHOR_DATE=$TODAY GIT_COMMITTER_DATE=$TODAY git commit --amend -m "$@"
fi
...
# echo $N_DAYS_BEFORE # For test
# git add .
# GIT_AUTHOR_DATE=$N_DAYS_BEFORE GIT_COMMITTER_DATE=$N_DAYS_BEFORE git commit --amend
